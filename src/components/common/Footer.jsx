...
export default function Footer() {
  return (
    <footer className="relative overflow-hidden bg-[#080808] border-t border-white/[0.05]">
      {/* Top glow accent */}
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[300px] h-16 bg-brand-orange/[0.04] blur-2xl pointer-events-none" />
      ...
      <div className="container-max relative z-10 px-4 sm:px-6 lg:px-8 pt-16 pb-8">
            ...
            <div className="flex items-center gap-2">
              {socials.map(({ label, href, icon }) => (
                <a
                  ...
                  rel="noreferrer"
                  aria-label={label}
                  className="w-9 h-9 rounded-xl border border-white/[0.08] bg-white/[0.03] flex items-center justify-center text-text-muted hover:text-brand-orange hover:border-brand-orange/30 hover:bg-brand-orange/[0.06] transition-all duration-200"
                >
                  {icon}
                </a>
              ))}
            ...
            <ul className="flex flex-col gap-3 items-center lg:items-start">
              {company.map(({ label, path }) => (
                <li key={label}>
                  <Link
                    to={path}
                    className="group inline-flex items-center gap-2 text-text-muted text-sm hover:text-text-light transition-colors duration-200"
                  >
                    <span className="w-0 group-hover:w-3 h-px bg-brand-orange transition-all duration-300 overflow-hidden" />
                    {label}
                  </Link>
                </li>
              ))}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-6">
          <p className="text-text-muted text-xs order-2 sm:order-1">
            © {new Date().getFullYear()} Linkframe Technologies. All rights reserved.
          </p>
          <div className="flex items-center gap-1.5 order-1 sm:order-2">
            <span className="w-1.5 h-1.5 rounded-full bg-brand-orange animate-pulse" />
            <p className="text-text-muted text-xs">Built with precision &amp; purpose.</p>
          </div>
        </div>
      </div>
    </footer>
